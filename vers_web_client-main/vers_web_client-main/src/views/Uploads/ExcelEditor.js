...
const ExcelEditor = () => {
  ...
  const onDataChange = useCallback((props) => {
    const { rowIndex, colIndex, updatedValue } = props;
    setFileData((prevData) => {
      const copyData = [...prevData];
      if (copyData[rowIndex]) {
        const copyRow = [...copyData[rowIndex]];
        copyRow[colIndex] = updatedValue;
        copyData[rowIndex] = copyRow;
      }
      return copyData;
    });
  }, []);

  const onDeleteData = useCallback(async (props) => {
    const { type, colIndex, rowIndex } = props;
    switch (type) {
      case "column":
        setFileData((prevData) => 
          prevData.map((innerArray) => innerArray.filter((_, index) => index !== colIndex))
        );
        setDefaultFileHeader((prevHeader) => 
          prevHeader.filter((_, index) => index !== colIndex)
        );
        break;
      case "row":
        setFileData((prevData) => 
  ...
  const handleMapColumnsClick = () => {
    if (headerMatchSummary.unmatchedColumns.length > 0) {
      toast.error(`Please map ${headerMatchSummary.unmatchedColumns.length} unmatched column(s).`);
    } else if (headerMatchSummary.duplicateMappedKeys.length > 0) {
      toast.error(`Duplicate mapped fields found: ${headerMatchSummary.duplicateMappedKeys.join(', ')}`);
    } else if (headerMatchSummary.totalColumns === 0) {
      toast.info("No file uploaded yet to map columns.");
    } else {
      toast.success("All columns are mapped correctly!");
    }
  };
  ...
  return (
    <>
      <div className="panel-header upload-sheet-toolbar upload-toolbar-controls p-2 min-h-12 rounded-xl flex flex-wrap gap-2 text-[#1c3a63] border border-[#d9e5f6] mb-2">
        ...
        <button 
          onClick={handleMapColumnsClick}
          className="flex items-center justify-center font-semibold text-sm px-4 py-2 border rounded-md border-[#cfdfef] bg-[#eef4f9] text-[#22446f] hover:bg-[#dfeaf5] transition-all duration-200"
        >
          Map Columns
        </button>
        <VerifyButton
